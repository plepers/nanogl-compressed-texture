...
function _getFormat( fid ){
  ...
  switch (fid) {
    ...
    default:
      throw new Error('unsupported format ' + fid);
  }
...
function _parse (arrayBuffer) {
  ...
  if (cubemap) {
    for (var f = 0; f < 6; f++) {
      ...
      for (i = 0; i < requiredMipLevels; i++) {
        dataLength = width * height * bpp;
        mips.push( new Uint8Array( arrayBuffer, dataOffset, dataLength ) );
